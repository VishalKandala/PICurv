...
/**
 * @brief Opens a per-run diagnostics CSV in the run's log directory for appending.
 *
 * The runtime diagnostics files share a lifecycle rather than a schema: they live in
 * `simCtx->log_dir`, are appended to across a run, carry their column header only once,
 * and mark the seam where a continuation resumed so a reader can tell a restart from a
 * discontinuity in the data. This owns that lifecycle; the caller owns the columns.
 *
 ...
 *
 * @param[in]  simCtx   Simulation context supplying the log directory, step, and
 *                      continuation state.

...
- `<repo>/logs/build.log`: full compiler/linker output generated by `make audit-build` or `picurv build`.
- `<repo>/logs/build.warnings.log`: warning-only extract generated by `make audit-build`.
- `<repo>/logs/doxygen.warnings`: Doxygen warning output configured by `docs/Doxyfile`.
- ad-hoc local logs created by runtime or tooling commands during diagnostics.
...
3. Fix root causes in source, docs, runtime packages, or tooling instead of suppressing warnings.

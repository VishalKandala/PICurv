...
- `simulator.c`: main entry for the flow solver executable.
- `postprocessor.c`: main entry for postprocessing executable.

## Major Module Files

- setup and lifecycle: `setup.c`, `runloop.c`
...
3. Keep high-level orchestration in `solvers.c`/`runloop.c`; keep kernels in module files.

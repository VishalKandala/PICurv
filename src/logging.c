...
/**
 * @brief Internal reduction callback for packed search metrics.
...
const char* MomentumSolverTypeToString(MomentumSolverType SolverFlag)
{
    switch(SolverFlag){
        ...
        case MOMENTUM_SOLVER_NEWTON_KRYLOV: return "Newton Krylov";
...
/**
 * @brief Implementation of \ref ResetSearchMetrics().
 * @details Full API contract (arguments, ownership, side effects) is documented with
 *          the header declaration in `include/logging.h`.
 * @see ResetSearchMetrics()
 */
...
/**
 * @brief Implementation of \ref LOG_SEARCH_METRICS().
 * @details Full API contract (arguments, ownership, side effects) is documented with
 *          the header declaration in `include/logging.h`.
 * @see LOG_SEARCH_METRICS()
 */
                                        ...
                                        const char *header, FILE **file)
{
    ...
    PetscCall(PetscSNPrintf(path, sizeof(path), "%s/%s", simCtx->analysis_dir, filename));
    handle = fopen(path, "a");

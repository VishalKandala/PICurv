...
The runtime writes `<run.analysis.metrics>/search_metrics.csv` for all particle-enabled runs.
...
`<run.analysis.metrics>/search_metrics.csv` is written once per timestep after particle settlement.

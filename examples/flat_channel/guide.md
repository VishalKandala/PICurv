...
`init` creates the case directory with config files — no binaries are copied. Runtime executables are resolved from the project `bin/` directory via PATH. `init` records source linkage in `.picurv-origin.json` and writes an inert `.picurv-execution.yml` for later site-specific launcher overrides. To pin specific binary versions, pass `--pin-binaries` to `picurv init`.

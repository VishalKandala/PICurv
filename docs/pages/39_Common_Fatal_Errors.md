...
Cross-file combinations that no single file can rule out are reported the same way:
...
| `Local x-width of domain x 2 is smaller than stencil width s 3` (or the y/z variants) | `poisson_solver.multigrid.levels` is too deep for the MPI rank layout. The coarsest level halves down to fewer nodes per rank than the stencil width, which is 3 whenever any axis is periodic. Aborts during DM creation, before the first timestep. | Lower `multigrid.levels` and delete the now-unused `level_solvers.level_N` entry, or reduce the rank count. Constraint is `floor((N / 2^(levels-1) + 1) / ranks_per_axis) >= stencil_width`; worked maxima are tabulated in @ref 25_Pressure_Poisson_GMRES_Multigrid. |

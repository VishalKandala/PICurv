...
- `<repo>/logs/doxygen.warnings` (generated by `make build-docs`)
...
- Stable outcomes should be moved into `<repo>/config/`, `examples/`, `picurv_cli/`, `generators/`, or `docs/`.
...
- move the local wrapper stream log out of `<run.runtime_logs>/` so it is not affected by solver-side log-directory recreation; `<run.scheduler>/` is the existing precedent for batch stdout/stderr.

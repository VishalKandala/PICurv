...
| `INLET` | `prescribed_flow` | `source.type: file`, `source.path` | `source.type: generated`, `source.generator`, `source.params` | m/s scalar speeds in `.picslice` or generated profile params |
...
`prescribed_flow` uses a face-scoped canonical `PICSLICE` file, either supplied
directly with `source.type: file` or generated by `picurv` with
`source.type: generated`. The Python conductor resolves the face/grid context
and delegates the dimensional profile write to `scripts/profile.gen`:
